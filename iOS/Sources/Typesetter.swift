struct Typesetter {
    ...
    func textViews(frame:CGRect, prepareForScrollView:Bool = true)->[UITextView] {
        ...
        let numberOfViews = Int(ceil(textStorage.boundingRectWithSize(CGSize(width: frame.width, height: frame.height-inset), options: .UsesFontLeading | .UsesLineFragmentOrigin, context: nil).height / (frame.height-inset)))
        ...
        for i in 0...numberOfViews {
            // create text container, make slightly smaller so it's inset from the provided frame size
            ...
            // create text view
            let textView = UITextView(frame: CGRect(x: xposition, y: 0, width: frame.width, height: frame.height), textContainer: layoutManager.textContainers[i] as? NSTextContainer)
        ...
        // return the array
        println(textViews.last?.text.lengthOfBytesUsingEncoding(NSUTF8StringEncoding))
        return textViews
    }
}
